...
static int get_my_obj_path(uint64_t oid, char *path) {
	
	return snprintf(path, PATH_MAX, "/vpublic02/private/obj/%016" PRIx64, oid);
}
...
int default_read(uint64_t oid, const struct siocb *iocb)
{
	...
	if (ret == SD_RES_NO_OBJ) {
		//PRINT_TO_LOG("SD_RES_NO_OBJ\n");
		get_my_obj_path(oid, path);
		ret = default_read_from_path(oid, path, iocb);
		PRINT_TO_LOG("MISS\n");
		PRINT_TO_LOG("%s\n", path);
