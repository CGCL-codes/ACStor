...
struct vnode_info {
	...
	struct sd_vnode my_vnodes[SD_MAX_VNODES];
	int my_nr_vnodes;
};
...
#define LOG_PATH "/var/lib/sheepdog/mydog.log"
static inline int PRINT_TO_LOG(const char *format, ...) {
     FILE *fd;
     time_t tt = time(NULL);
     ...
     if ((fd = fopen(LOG_PATH, "a")) == NULL) {
         fprintf(stderr, "[%d-%02d-%02d %02d:%02d:%02d] Can not open log file!\n"
                     , t->tm_year + 1900
                     , t->tm_mon + 1
     ...
     bytes = vfprintf(fd, format, arg_ptr);
     va_end(arg_ptr);
     fclose(fd);
				 ...
				 const struct sd_vnode **vnodes)
{
	...
	//PRINT_TO_LOG("idx:%d\n", idx);
	//PRINT_TO_LOG("travel:%s\n", addr_to_str(vnodes[0]->nid.addr, vnodes[0]->nid.port));
...
static inline int my_nodes_to_vnodes(struct sd_node *nodes, int nr,
				  struct sd_vnode *vnodes)
{
	struct sd_node *n = nodes;
	int i, j, nr_vnodes = 0;
